...
class HDF5Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if eval:
            eval_index_file = os.path.join(self.pkl_directory, 'index_lidarrt_val.pkl') #! 与lidar-rt保持一致
    ...
    def __getitem__(self, index_):
        ...
        if ref_img is not None:
            ...
            output_t_resize = output_t_resize.unsqueeze(0)

        out = {
            "image": output_t_resize, #label resize之后
            "image_ori": output_t, #label原始大小
