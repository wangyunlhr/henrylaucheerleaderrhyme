...
#! 
from lidm.data.kitti import HDF5Dataset
...
def batch_range2point_from_batch(batch, range_maps=None, resize_flag=False, device="cuda"):
    ...
    for i in range(B):
        ...
        points_hw3 = range2point(
            range_map=range_map_i,           # (H,W)
            sensor_center=sensor_center_i,   # (3,)
            sensor2world=sensor2world_i,     # (4,4)（你现在注释掉世界变换也没关系）
            ir=ir_i,
            angle_offset=angle_offset_i,
            pixel_offset=pixel_offset_i,
            mask = range_mask_i #!已经用mask处理一遍了
...
def single_range2point_from_sample(sample, index, range_map=None, device="cuda"):
    ...
    points_hw3 = range2point(
        range_map=range_map,           # (H,W)
        sensor_center=sensor_center,   # (3,)
        sensor2world=sensor2world,     # (4,4)
        ir=ir,
        angle_offset=angle_offset,
        pixel_offset=pixel_offset,
        mask=range_mask,               # 如果你在 range2point 里用这个做过滤
    )
...
if __name__ == "__main__":
    ...
    if not opt.baseline: #! 
        base_configs = [f'{logdir}/fixed_based_sam.yaml']
    else:
    ...
    if opt.file is None:
        ...
        dataset = HDF5Dataset(
            ...
            data_path=config.data.params.dataset_path,
            tokenizer=None,
            eval=True  # 表示验证 / 测试模式
        )
        ...
        dataloader = DataLoader(
            ...
            shuffle=False,
            drop_last=False,
            collate_fn=test_collate_fn,   # 如果 HDF5Dataset 返回的字典结构和原来一致，collate_fn 可以复用
        )
